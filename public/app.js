...
function closeMentionDropdown() {
    mentionDropdown.style.display = 'none'; filteredUsers = [];
...
function setReply(data, previewText) {
    replyingToMessage = { id: data.id, user: data.user, text: previewText.length > 50 ?
...
socket.on('update reaction', function(data) {
    ...
    if (count > 0) {
        btn.classList.add('has-reactions'); tooltip.innerHTML = 
data.users.map(u => `<div class="tooltip-row">${u}</div>`).join('');
    } else {
